...
- `./analyse.py` can be used to create various plots and reports from the data generated by the above tools
    - when indexing job data, you may need to issue manual instructions to resolve inconsistencies and duplicate log files; see `tips:.override files` below...
    - a successful index will create a .plan file for each set of jobs with the same parameters
        - identifying job parameters is done using tolerances defined in analyse.py
            - this means that jobs with slightly different parameters might be grouped together
            - this may or may not be what you want...
            - currently the best you can do is to adjust the global tolerance parameters
        - if similar jobs should be grouped, then the name of the plan will correspond to an arbitrary job's parameters
            - you are free to rename the plan file to any name whose corresponding parameters fall within the tolerance
        - plans are initially 'staged' or held, meaning that you should manually review the file to check the description is acceptable
            - if so, remove the 'staged' key or set it to `false`
            - otherwise, feel free to edit the file as desired
            - the file contains a `sequence` key which contains an array of all matching jobs
                - all jobs *must* be included here; if not, `./analyse.py` will re-insert them and restage the plan
                - if you want to exclude a job, you should set the range to empty
                - the job parameter corresponds to the job basename
                - the range parameter has format "[start]:[stop]", where start and stop are both optional indices into the outp (.csv) file
            - the order of the jobs is the order in which the data will be read in
                - you can put a job in multiple times if for some reason you'd like to read parts of that job at different points
                    - it is up to you to make sure individual datums are not read twice
                        - to do so, use mutually exclusive ranges...
                    - there is not much reason to do this though...
...
#### .override files
When using `./analyse.py`, you may encounter inconsistencies in your job files. These may arrise due to a number of circumstances:
- jobs may have been begun using earlier versions of `./walk` and associated utilities, resulting in missing files or output entries
- jobs may have crashed or been interrupted leading to inconsistencies between various files (less likely in recent versions)
- jobs may have been run manually leading to missing files or inconsistent naming conventions

Some of these should be resolved by manually editing the relevant files to correct inconsistencies (for example, earlier versions of `./walk` generated .csv files in which the third column was `n` rather than the total number of iterations; it is fairly safe to use the values suggested by `./analyse.py index` as replacements). Some inconsistencies are better addressed by manual overrides of certain warnings however.

To override a warning due to a record in the outp(.csv) file but not in the log file, add entries like the following to the override file:

    outp:3
    outp:7-12
    outp:4,9,13-17,20

where the numbers and ranges are the reported indices of the outp record whose warning you would like to suppress.
